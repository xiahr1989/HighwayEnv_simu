...
class RuleHierarchy(AbstractRuleHierarchy, Generic[T]):
    ...
    #         return cost
    def traj_cost(
        self, traj: torch.Tensor, selected_rule_name: str = None, get_robustness_vector: bool = False
    ):
        self._device_check(traj)

        all_robustness = self._traj_robustness(traj)

        if selected_rule_name:
            selected_indices = [
                i for i, rule in enumerate(self.rules) if rule.__class__.__name__ == selected_rule_name
            ]
            if not selected_indices:
                raise ValueError(f"Rule '{selected_rule_name}' not found in RuleHierarchy!")

            robustness_vector = all_robustness[:, selected_indices]
            scaling = self.scaling[selected_indices]
        else:
            robustness_vector = all_robustness
            scaling = self.scaling

        cost, scaled_robustness_vector = self._cost(robustness_vector, scaling)

        return (cost, scaled_robustness_vector) if get_robustness_vector else cost
